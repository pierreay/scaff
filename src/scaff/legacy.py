...
def align(
    template, target, sr, ignore=True, log=False, get_shift_only=False, normalize=False
):
    ...
    assert (
        template.ndim == 1 and target.ndim == 1
    ), "Traces to align should be 1D-ndarray!"
...
def get_comp(traces, comp):
    ...
    assert (
        type(comp) == str or comp in CompType
    ), "COMP is set to a bad type or bad enum value!"
    if (type(comp) == CompType and comp == CompType.AMPLITUDE) or (
        type(comp) == str and CompType[comp] == CompType.AMPLITUDE
    ):
        return get_amplitude(traces)
    elif (type(comp) == CompType and comp == CompType.PHASE) or (
        type(comp) == str and CompType[comp] == CompType.PHASE
    ):
        return get_phase(traces)
    elif (type(comp) == CompType and comp == CompType.PHASE_ROT) or (
        type(comp) == str and CompType[comp] == CompType.PHASE_ROT
    ):
...
def is_p2r_ready(radii, angles):
    ...
    radii_interval = radii[radii < 0].shape == (0,) and radii[
        radii > np.iinfo(np.uint16).max
    ].shape == (0,)
    # Check that -PI <= ANGLES <= PI.
    angles_interval = angles[angles < -np.pi].shape == (0,) and angles[
        angles > np.pi
    ].shape == (0,)
...
def normalize(arr, method=NormMethod.MINMAX, arr_complex=False):
    ...
    if arr_complex is True:
        ...
        arr_polar = r2p(arr)
        return normalize(arr_polar[0], method=method), normalize(
            arr_polar[1], method=method
        )
    else:
        assert (
            arr.dtype == np.float32 or arr.dtype == np.float64
        ), "normalization input should be floating points numbers"
        if method == NormMethod.MINMAX:
            ...
            return normalize_generic(
                arr,
                {
                    "actual": {"lower": arr.min(), "upper": arr.max()},
                    "desired": {"lower": 0, "upper": np.iinfo(np.int16).max},
                },
            )
            ...
            return normalize_generic(
                ...
                {
                    "actual": {"lower": arr.min(), "upper": arr.max()},
                    "desired": {"lower": -np.pi, "upper": np.pi},
                },
            )
...
def find_starts(config, data):
    ...
    trigger = butter_bandpass_filter(
        data, config.bandpass_lower, config.bandpass_upper, config.sampling_rate, 6
    )
    ...
    # lowpass values:
    lpf = signal.butter(
        5, config.lowpass_freq, "low", fs=config.sampling_rate, output="sos"
    )
    ...
    if config.trigger_threshold is not None and config.trigger_threshold > 0:
        ...
        )  # NOTE: / 100 because of *100 in plot_results().
    ...
    starts = (
        np.where((trigger_signal[1:] != trigger_signal[:-1]) * trigger_signal[1:])[0]
        + start_idx
        + offset
        + 1
    )
...
class ExtractConf(config.ModuleConf):
    ...
    def load(self, appconf):
        ...
        self.num_traces_per_point_min = self.get_dict(appconf)[
            "num_traces_per_point_min"
        ]
        self.num_traces_per_point_max = self.get_dict(appconf)[
            "num_traces_per_point_max"
        ]
...
def extract(trace, template, config, average_file_name=None, results_old=None):
    ...
    if template is not None and len(template) != trace_length:
        raise Exception(
            "Template length doesn't match collection parameters: {} != {}".format(
                len(template), trace_length
            )
        )

    # Create starts based on trigger frequency or restore previous trace starts.
    if results_old is None:
        results_new.trace_starts, results_new.trigger, results_new.trigger_avg = (
            find_starts(config, trace)
        )
    else:
    ...
    for start_idx, start in enumerate(results_new.trace_starts):
        ...
        if results_old is None:
            ...
            trace_lpf = butter_lowpass_filter(
                trace_candidate, config.sampling_rate / 4, config.sampling_rate
            )
            template_lpf = butter_lowpass_filter(
                template, config.sampling_rate / 4, config.sampling_rate
            )
            ...
            if config.min_correlation > 0 and max(correlation) < config.min_correlation:
                l.LOGGER.debug(
                    "Skip trace start: {} < {}".format(
                        max(correlation), config.min_correlation
                    )
                )
    ...
    # Print the results.
    l.LOGGER.info(
        "num_traces_per_point > starts > extracted > min ; skip_by_corr : {} > {} > {} > {} ; {}".format(
            num_traces_per_point,
            len(results_new.trace_starts),
            len(results_new.traces),
            config.num_traces_per_point_min,
            results_new.skip_nb,
        )
    )
    if len(results_new.corrs) != 0:
        l.LOGGER.info(
            "percentile(corrs) : 1% / 5% / 10% / 25% : {:.2e} / {:.2e} / {:.2e} / {:.2e}".format(
                np.percentile(results_new.corrs, 1),
                np.percentile(results_new.corrs, 5),
                np.percentile(results_new.corrs, 10),
                np.percentile(results_new.corrs, 25),
            )
        )
    ...
    if np.shape(avg) == ():
        ...
        raise Exception(
            "Not enough traces have been averaged: {} < {}".format(
                len(results_new.traces), config.num_traces_per_point_min
            )
        )
...
def load_all(filename, number=0):
    ...
    return [[int(c) for c in bytearray.fromhex(line)] for line in data.split("\n")]
...
def generic_load(
    data_path, number, wstart=0, wend=0, norm=False, norm2=False, comp="amp"
):
    """
    ...
    for i in range(number):
        ...
        raw_traces = np.load(path.join(data_path, "%d_%s.npy" % (i, comp)))
...
def compute_variables(variable):
    ...
    if variable == "c":
        ...
        for bnum in range(NUM_KEY_BYTES):
            VARIABLES[bnum] = list(
                map(VARIABLE_FUNC, PLAINTEXTS[:, bnum], KEYS[:, bnum])
            )
...
def estimate_ttest():
    ...
    for bnum in range(NUM_KEY_BYTES):
        TTESTS[bnum], PTTESTS[bnum] = ttest_ind(
            SETS[bnum][1], SETS[bnum][0], axis=0, equal_var=False
        )
...
def split(fold, k_fold):
    ...
    test_range = [
        i for i in range(0, Ntraces) if i >= fold * Ntest and i < fold * Ntest + Ntest
    ]
    profiling_range = [
        i for i in range(0, Ntraces) if i < fold * Ntest or i >= fold * Ntest + Ntest
    ]
...
def estimate_corr():
    ...
    for bnum in range(NUM_KEY_BYTES):
        ...
        print(
            ...
            "min: ",
            np.min(CORRS[bnum]),
            -np.log10(PS[bnum][np.argmin(CORRS[bnum])]),
        )
        print(
            "byte",
            bnum,
            "max: ",
            np.max(CORRS[bnum]),
            -np.log10(PS[bnum][np.argmax(CORRS[bnum])]),
        )
...
def find_pois(pois_algo, k_fold, num_pois, poi_spacing, template_dir="."):
    ...
    if PLOT or SAVE_IMAGES:
        ...
        if pois_algo == "r":
            ...
            plt.title(
                "%d-folded r-test: r_z = 0.5*ln((1+r)/(1-r)) / (1/sqrt(Ntrace-3))"
                % k_fold
            )
            plt.xlabel("samples")
            plt.ylabel("r_z")
        ...
        if SAVE_IMAGES:
            ...
            plt.savefig(
                os.path.join(template_dir, "pois.pdf"), bbox_inches="tight", dpi=100
            )
        if PLOT:
            plt.show()
        plt.clf()
    ...
    if PLOT or SAVE_IMAGES:
        for i in range(num_pois):
            ...
            if SAVE_IMAGES:
                ...
                plt.savefig(
                    os.path.join(template_dir, "profile_poi_%d.pdf" % i),
                    bbox_inches="tight",
                    dpi=100,
                )
            if PLOT:
...
def fit(lr_type, variable):
    ...
    if lr_type:
        if lr_type == "linear":
            ...
            raise Exception("Linear regression type %s is not supported" % lr_type)
    else:
        return
    ...
    for bnum in range(NUM_KEY_BYTES):
        ...
        for cla in CLASSES:
            for i in range(num_pois):
                ...
                PROFILE_MEANS_FIT[bnum][cla][i] = sum(betas[0:num_betas] * models[cla])
    if PLOT:
        for i in range(num_pois):
            plt.xlabel(variable)
            plt.ylabel("normalized amplitude")
            ...
            plt.plot(CLASSES, PROFILE_MEANS_FIT[0][:, i], "r-", label="fit")
            plt.errorbar(
                CLASSES,
                PROFILE_MEANS[0][:, i],
                yerr=PROFILE_STDS[bnum][:, i],
...
def save_profile(template_dir):
    ...
    np.save(path.join(template_dir, "PROFILE_MEAN_TRACE.npy"), PROFILE_MEAN_TRACE)
...
def run_attack(
    attack_algo, average_bytes, num_pois, pooled_cov, variable, retmore=False
):
    ...
    if attack_algo == "pdf":
        ...
        if num_pois > len(PROFILE_COVS[0][0][0]):
            print(
                "Error, there are only %d pois available" % len(PROFILE_COVS[0][0][0])
            )
        ...
        assert (
            len(POIS[0]) >= num_pois
        ), "Requested number of POIs (%d) higher than available (%d)" % (
            num_pois,
            len(POIS[0]),
        )
        ...
        for bnum in range(0, NUM_KEY_BYTES):
            ...
            for kguess in range(256):
                ...
                clas = [
                    VARIABLE_FUNC(PLAINTEXTS[j][bnum], kguess)
                    for j in range(len(TRACES))
                ]
                if average_bytes:
                    leaks = np.asarray(
                        [PROFILE_MEANS_AVG[clas[j]] for j in range(len(TRACES))]
                    )
                else:
                    leaks = np.asarray(
                        [PROFILE_MEANS[bnum][clas[j]] for j in range(len(TRACES))]
                    )
...
def rank():
    ...
    rank_min, rank_rounded, rank_max, time_rank = hel.rank(
        LOG_PROBA, known_key, merge, bins
    )
...
def bruteforce(bit_bound_end):
    ...
    found = hel.bruteforce(
        LOG_PROBA, pt1, pt2, ct1, ct2, merge, bins, bit_bound_start, bit_bound_end
    )
...
def attack(
    ...
    found = run_attack(attack_algo, average_bytes, num_pois, pooled_cov, variable)
...
def attack_recombined(
    ...
    def attack_comp(
        ...
        if align is True:
            ...
            l.LOGGER.info("Align attack traces with the profile...")
            TRACES = align_all(
                TRACES, int(fs), template=PROFILE_MEAN_TRACE, tqdm_log=True
            )
        ...
        reduce_traces(num_pois, window)
        maxcpa = run_attack(
            attack_algo, average_bytes, num_pois, pooled_cov, variable, retmore=True
        )
    ...
    comp = "amp"
    maxcpa[comp] = attack_comp(
        comp,
        template_dir.format(comp),
        variable,
        pois_algo,
        num_pois,
        poi_spacing,
...
def tra_create(
    ...
    for knum in range(NUM_KEY_BYTES):
        if fixed_key:
            tempSbox = [
                sbox[PLAINTEXTS[i][knum] ^ tempKey[0][knum]] for i in range(len(TRACES))
            ]
        else:
            tempSbox = [
                sbox[PLAINTEXTS[i][knum] ^ tempKey[i][knum]] for i in range(len(TRACES))
            ]
        ...
        for HW in range(9):
            assert len(tempTracesHW[HW]) != 0, (
                "No trace with HW = %d, try increasing the number of traces" % HW
            )
...
def tra_attack(
    ...
    for knum in range(0, NUM_KEY_BYTES):
        ...
        for j, trace in enumerate(TRACES):
            ...
            if all(k == atkKey[knum] for k in window) or (
                j == len(TRACES) - 1 and guessed == atkKey[knum]
            ):
